...
This reference covers how to query the Dagster Plus GraphQL API to retrieve automation condition evaluation records.
...
Query all asset evaluations that happened during a specific evaluation tick (across multiple assets).
...
- **`numCandidates`**: Count of partitions/assets evaluated (null for non-partitioned, or when all partitions considered)
